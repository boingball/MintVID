...
# DV/NTSC (4:1:1), 720x480 - libdv has no planar 4:1:1 renderer, only a
# packed-YUY2 one; core/mr_dv.c downsamples that itself to reach
# MR_PIX_YUV420P, hence the higher (but still passing) MAE than the PAL
# case above - see that file's own header.
ffmpeg -v error -f lavfi -i testsrc2=size=720x480:rate=30000/1001:duration=1 \
    -c:v dvvideo -pix_fmt yuv411p test_dv_ntsc.avi -y
...
rm -rf ref_dv_ntsc && mkdir -p ref_dv_ntsc
ffmpeg -v error -i test_dv_ntsc.avi ref_dv_ntsc/f%03d.ppm -y
