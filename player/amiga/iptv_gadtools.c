...
int main(int argc,char **argv)
{
    ...
    while(!done){struct IntuiMessage *msg;ULONG signals=Wait(winmask|timermask|SIGBREAKF_CTRL_C);
      ...
      while((msg=GT_GetIMsg(app.window->UserPort))!=NULL){ULONG cls=msg->Class;UWORD code=msg->Code;struct Gadget *gad=(struct Gadget *)msg->IAddress;UWORD id=gad?gad->GadgetID:0;GT_ReplyIMsg(msg);
        ...
        else if(cls==IDCMP_MENUPICK){int action=mr_gui_menu_action(&app.menu,code);if(action==MR_GUI_MENU_ABOUT)mr_gui_show_about(app.window,"IPTV GadTools edition (OS 3.0)");else if(action==MR_GUI_MENU_GUIDE)mr_gui_open_guide(&app.menu,app.window);else if(action==MR_GUI_MENU_TLS)mr_http_set_tls_max(MR_TLS_PREF_HTTP_MAX());else if(action==MR_GUI_MENU_QUIT)done=1;}
