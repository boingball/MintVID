...
/* MintVID adaptation: see mr_dv_endian.h. */
#include "mr_dv_endian.h"
#include <stdlib.h>
...
/* ----------------------------------------------------------------------------
 */
/* MintVID adaptation: `pitches` narrowed from `int *` to `uint16_t *`,
 * matching dv_mb420_YV12()'s own already-adapted signature (see YV12.c/h
 * and dv.c) - mr_dv.c's one dv_render_macroblock_yuv() dispatch passes the
 * same uint16_t pitches[] array to whichever renderer the stream's
 * sampling picks, 420 or 411. */
void
dv_mb411_YUY2(dv_macroblock_t *mb, uint8_t **pixels, uint16_t *pitches, int add_ntsc_setup) {
...
/* ----------------------------------------------------------------------------
 */
/* MintVID adaptation: see dv_mb411_YUY2()'s own note above. */
void
dv_mb411_right_YUY2(dv_macroblock_t *mb, uint8_t **pixels, uint16_t *pitches, int add_ntsc_setup) {
