...
dv_decode_full_frame(dv_decoder_t *dv, const uint8_t *buffer,
  ...
  /* MintVID adaptation: dropped the pthread_mutex_t guard here - mrplay/
   * mr_decode only ever call this from one task at a time, so libdv's own
   * defence against concurrent decoder use (irrelevant to us) is unneeded,
   * and AmigaOS has no pthreads to link against anyway. Also narrowed
   * `pitches` from `int *` to `uint16_t *`, matching what dv_mb420_YV12()
   * (YV12.c/YV12.h) actually declares and reads - the original `int *pitches`
   * here was a latent type mismatch (dv_render_macroblock_yuv/DV_MB420_YUV
   * pass it straight through to a uint16_t* parameter) that just happened
   * to read correctly on little-endian hosts; on big-endian m68k it would
   * read the wrong 16 bits of every `int pitches[i]` slot. */
  ...
  for (ds=0; ds < dv->num_dif_seqs; ds++) {
	...
	*/
	...
	audio++;
      ...
      /* MintVID adaptation: color_space is always e_dv_color_yuv here (see
       * mr_dv.c) - the e_dv_color_bgr0/e_dv_color_rgb cases (and the
       * rgb.c-only RANGE_CHECKING diagnostic) are dropped along with
       * rgb.c/YUY2.c above. */
      for (m=0,mb = seg->mb;
	   m<5;
	   m++,mb++) {
